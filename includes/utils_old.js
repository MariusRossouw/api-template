...
var yodlee_3rdparty = require("../includes/yodlee_3rdparty.js");
...
function do_wf(pdb, wf_id, callback) {
    ...
    function do_job(){
        ...
        if (job.type == 'wf_test') {
            match_job_type = true;
            console.log('WF TEST SUCCESS: ', wf.payload);
            job.output = {simple_test: true};
            wrapup(null);
        };

        if (job.type == 'yodlee_action') {
            match_job_type = true;
            console.log('yodlee: ', wf.payload);
            yodlee_3rdparty.yodlee_generic(wf.payload, function(err, data){
                if(err){
                    console.log(err);
                    job.output_error = err;
                }
